...
#' chronostratigraphic timescale will be applied to the data. "ICS_2013"
#' will retain the PBDB ICS 2013 standard. "GTS_2020" will update all
#' early and late interval ages to the GTS_2020 standard, using a
...
get_pbdb <- function(taxon = NULL, interval = NULL, mode = "occurrence", res = "all", fields = c("ident", "coords", "class"),
                     ...
                     return_url = FALSE, return_data = TRUE, save_as = NULL, tscale = "ICS_2013", wait = Inf) {
  ...
  if(!is.null(interval)) {
    ...
    if(is.character(interval)) {
      ...
      GTS_2020 <- get("GTS_2020")
      interval <- c(GTS_2020$FAD[match(interval[1], GTS_2020$Interval)], GTS_2020$LAD[match(interval[2], GTS_2020$Interval)])
    }
